  ...
  <div class="login-container">
    ...
    <div class="login-header">
      <a :class="{active: loginway === 'login'}"
        @click="changeLogin('login')">&nbsp;&nbsp;&nbsp;&nbsp;登录&nbsp;&nbsp;&nbsp;&nbsp;
      </a>·
      <a :class="{active: loginway === 'register'}"
        @click="changeLogin('register')">&nbsp;&nbsp;&nbsp;&nbsp;注册&nbsp;&nbsp;&nbsp;&nbsp;
      </a>
    </div>
    <div class="login-input" v-if="loginway == 'login'">
      <form name="formLogin" class="form_login" accept-charset="UTF-8" method="post">
        ...
        <button type="submit" class="login-btn btn"><span>登录</span></button>
      ...
      <form name="formRegister" class="form_register" accept-charset="utf-8" method="post">
        <div class="email">
          ...
          <input type="email" name="email"
            v-model="user.email"
            required placeholder="请输入你的邮箱地址" class="span2">
        </div>
        <div class="username">
          <span class="span1"><i class="fa fa-user"></i></span>
          <input type="text" name="telephone"
            v-model="user.telephone"
            required placeholder="手机号码" class="span2">
        </div>
        <div class="password">
          <span class="span1"><i class="fa fa-unlock-alt"></i></span>
          <input type="password" name="password"
            v-model="user.password"
            required placeholder="密码" class="span2">
        </div>
        <div></div>
        <button class="register-btn btn" type="button"
          @click="register()"><span>注册</span></button>
        <p class="register_text">点击"注册"或下浮社交登录按钮，即表示您同意并愿意遵守简书 <a href="#">用户协议</a> 和 <a href="#">隐私政策</a> 。</p>
      </form>
    </div>
    <div class="login-way">
      <h5>您好可以通过以下方式登录</h5>
